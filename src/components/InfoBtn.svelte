...
<button class="opacity-50 hover:opacity-80 focus:scale-90" on:click={() => isOpen = !isOpen}>
	<span class="sr-only">Picture Info</span>
	...
	<div class="fixed top-0 left-0 w-full h-screen bg-gray-900/90 z-40" transition:fade="{{duration: 100}}">
		<div class="stack h-full">
			...
			<button on:click={() => isOpen = false} class="place-self-end opacity-50 hover:opacity-80 focus:scale-90 absolute top-5 right-5">
				<span class="sr-only">Close Info</span>
			...
			<div class="place-self-start sm:place-self-end sm:bg-gradient-to-t from-black w-full">
				<img src={image_url} class="mt-20 sm:hidden sm:h-full w-full object-contain max-w-6xl" alt={title}/>
				<div class="main w-full flex justify-between items-end sm:items-center">
					<div class="text-gray-300">
						<h2 class="text-xl">{title}</h2>
						<p class="text-lg"
							>{description}</p>
						<a class="underline" href={url}>» more info</a>
					</div>
					<div>
						<a href={location_url} class="opacity-20 hover:opacity-80 focus:scale-90">
